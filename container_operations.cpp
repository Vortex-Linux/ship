...
void create_container() {
    ...
    if (ship_env.image.empty()) {
        std::cout << "Please specify the image/distro which container should be based on(By default the image/distro will be set to Arch Linux.\nAll the image names can be found at the containers distros section in https://distrobox.it/compatibility/#containers-distros): ";
        getline(std::cin, ship_env.image);
        if (ship_env.image.empty()) {
